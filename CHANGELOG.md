...
## [v24] 2026-02-27 — Remove Siemens-specific hardcoding from import engine

### Fixed — Import Engine
- **Comments de-branded** — All import function comments referencing "Siemens" updated to "Viewpoint Vista"; column mappings were always standard Vista export format, not project-specific
- **Dead fallback code removed** — `vpImportJobCostMain`: absolute column index fallbacks (`colEst=19`, `colJTD=31`, `colProj=39`, `colCommit=57`) were unreachable dead code placed after a `continue` guard; removed to prevent confusion
- **`vpImportContract`** — "Siemens layout" comment updated to "Viewpoint Vista layout"
- **`vpImportARaging`** — "Row structure (Siemens actual)" updated to "Viewpoint Vista row structure"
- **README** — Import engine section heading no longer references "Siemens XLS"
- **CHANGELOG v23** — Entry title corrected to "Viewpoint Vista Import Engine"
- **DATA_MODEL** — Example `projectName` generalized away from hardcoded "Siemens - Wendell"

### Notes
- No logic changes — comment corrections and dead-code removal only
- Import column mappings were already correct for any Viewpoint Vista export

---

## [v23] 2025-02-27 — Viewpoint Vista Import Engine + AR Billing Dashboard
